...
dates_row_number <- df[which(df$Dates == '20180102'):which(df$Dates == '20220630'),] %>% select(Dates, row_number)
...
#for loop to derive a list of set of betas of industries for every date within data
...
w_ew <- get_ew_weights(get_weights(df, 48, start_date_index))
for (i in start_date_index:end_date_index){
  day_weights_lst <- get_weights(df, 48, i)
  lasso_weights_lst <- append(lasso_weights_lst, get_lasso_weights(day_weights_lst))
  ridge_weights_lst <- append(ridge_weights_lst, get_ridge_weights(day_weights_lst))
  minvar_weights_lst <- append(minvar_weights_lst, get_minvar_weights(day_weights_lst))
}
end_time <- Sys.time()
...
daily_returns <- function(weights_lst, initial_df, dates_df, first_date_index){
  # weights_lst = lasso_weights_lst
  # initial_df = df
  # dates_df = dates_row_number
  ...
  for (i in 1:nrow(dates_df)){
    ...
    day_return <- crossprod(weights_lst[[i]], as.numeric(as.vector(df[date_index, 2:ncol(df)])))
...
daily_ew_returns <- function(equally_weighted_matrix, initial_df,  dates_df, first_date_index){
  #initialise date index
  date_index = first_date_index
  #function to return vector of daily returns
  #initialise vector of returns
  ...
  for (i in 1:nrow(dates_df)){
    i = 1
    #sumproduct weights and individual returns
    day_return <- crossprod(equally_weighted_matrix, as.numeric(as.vector(df[date_index, 2:ncol(df)])))
    #append weighted return
    returns <- append(returns, day_return)
    date_index <- date_index + 1 #increase date index
  }
...
```{r}
lasso_daily_returns <- daily_returns(lasso_weights_lst, df, dates_row_number, start_date_index)
ridge_daily_returns <- daily_returns(ridge_weights_lst, df, dates_row_number, start_date_index)
ew_daily_returns <- daily_ew_returns(w_ew, df, dates_row_number, start_date_index)
minvar_daily_returns <- daily_returns(minvar_weights_lst, df, dates_row_number, start_date_index)
...
## creting df of returns for all models
```{r}
