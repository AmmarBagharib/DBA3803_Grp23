...
# To identify the row that correspond to the date
end_row <- which(data$Dates == "20220103") 
start_row <- which(data$Dates =="20120103")  # To get the starting data to collect for the 63 observations

#convert dates column into date format
data <- data %>% mutate(Dates = ymd(as.character(Dates)))

#slice data to obtain 6 industries with respective 63 daily returns
...
#initialise vector of cumulative returns with initial investment of $1000 for both ridge and lasso
